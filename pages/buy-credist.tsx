...
export default async function Pricing(
  ...
  res: NextApiResponse) {
    ...
    const gsession = await getSession({ req });
    console.log(gsession)
  
    if (!gsession || !gsession.user) {
    ...
    const PaystackLinks = [
      ...
      `https://paystack.com/pay/-kylnxks08?email=${encodeURIComponent(gsession.user.email)}`,
      `https://paystack.com/pay/01tus9xbji?email=${encodeURIComponent(gsession.user.email)}`,
      `https://paystack.com/pay/o85e9hdfx1?email=${encodeURIComponent(gsession.user.email)}`, //max
    ];
  ...
  return (
    // Return the PaystackLinks array as response
    // res.status(200).json({ PaystackLinks });
